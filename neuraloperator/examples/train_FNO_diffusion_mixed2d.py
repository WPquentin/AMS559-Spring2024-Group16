...
from neuralop.models import TFNO, FNO1d_with_BC, FNO1d, FNO2d, FNO2d_with_BC
from neuralop import Trainer
from neuralop.datasets import load_diffusion_pt_mixed, load_diffusion_pt_withoutBC_mixed, load_diffusion_pt_2D_withoutBC_mixed, load_diffusion_pt_2D_mixed
...
@hydra.main(version_base = None, config_path="conf", config_name="config_mixed")
def train_model(cfg: DictConfig):
    ...
    if not using_BC:
        ...
        train_loader, test_loaders = load_diffusion_pt_2D_withoutBC_mixed(
        ...
        model = FNO2d(n_modes_height = n_modes, n_modes_width = n_modes, in_channels=1, hidden_channels=32, projection_channels=64, factorization=None, rank=0.42, domain_padding=domain_padding, norm=norm_type)
    else:
        print("Using FNO with BC!")
        train_loader, test_loaders = load_diffusion_pt_2D_mixed(
                train_ratio=0.9, batch_size=batch_size, test_batch_size=10, data_path_parent=data_path_par, data_names=dt_names, whether_shuffle=whether_shuffle
        )
        model = FNO2d_with_BC(n_modes_height = n_modes, n_modes_width = n_modes, in_channels=1, hidden_channels=32, projection_channels=64, factorization=None, rank=0.42, domain_padding=domain_padding, norm=norm_type)
    ...
    # Creating the losses
    l2loss = weighted_LpLoss(d=2, p=2)
    h1loss = H1Loss(d=2)
