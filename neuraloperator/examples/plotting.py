...
def MakePlots_history(x, BackwardEuler_history, nt, dt, k0,
    xlim=[0,1], ylim=[0,4]):
    ...
    """
    n = len(BackwardEuler_history)
    fig, ax = plt.subplots(nrows = 2, ncols = n // 2 +1)

    for i in range(n):
        row = i % 2
        col = i // 2
        ax[row, col].plot(x, BackwardEuler_history[i], color='b')
        title = r'$ t=%.1f$ s' % ( i * 10 * dt)
        # title += r', $k_0 = %.1f\times 10^{-3}$ m$^2$s$^{-1}$' % (1000*k0)
        ax[row, col].set_title(title, fontdict = {'fontsize': 10})

        ax[row, col].set_xlim(xlim)
        ax[row, col].set_ylim(ylim)


        ax[row, col].set_xlabel(r'$x$ (m)', fontsize = 12)
        # ax[row, col].set_ylabel(r'$q(x, t=%.1f$ s$)$' % (nt*dt), fontsize = 12 )
...
def MakePlots(x, initial, CrankNicolson, BackwardEuler, nt, dt, k0,
    xlim=[0,1], ylim=[0,4]):
    """Plot the sample run shown in the documentation.
