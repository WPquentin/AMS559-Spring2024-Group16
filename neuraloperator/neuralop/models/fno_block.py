...
class FactorizedSpectralConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_modes, incremental_n_modes=None, bias=True,
        ...
        if bias:
            ...
            self.bias_bc = nn.Parameter(scale * torch.randn(*((n_layers, self.out_channels) + (1, )*self.order)))
        else:
            self.bias = None
...
class FactorizedSpectralConv1d_bc(FactorizedSpectralConv):
    def forward(self, x, bc, indices=0):
        # Here, bc shape should be batchsize, 1, 1
        batchsize, channels, width = x.shape

        x = torch.fft.rfft(x, norm=self.fft_norm)
        bc = torch.fft.rfft(bc, norm=self.fft_norm)
        
        out_fft = torch.zeros([batchsize, self.out_channels,  width//2 + 1], device=x.device, dtype=torch.cfloat)
        out_fft[:, :, :self.half_n_modes[0]] = self._contract(x[:, :, :self.half_n_modes[0]], self._get_weight(indices), separable=self.separable)

        out_bc = torch.zeros([batchsize, self.out_channels,  width//2 + 1], device=x.device, dtype=torch.cfloat)
        out_bc[:, :, :self.half_n_modes[0]] = self._contract(bc[:, :, :self.half_n_modes[0]], self._get_weight(indices), separable=self.separable)

        x = torch.fft.irfft(out_fft, n=width, norm=self.fft_norm)
        bc = torch.fft.irfft(out_bc, n=width, norm=self.fft_norm)

        if self.bias is not None:
            x = x + self.bias[indices, ...]
            bc = bc + self.bias_bc[indices, ...]
