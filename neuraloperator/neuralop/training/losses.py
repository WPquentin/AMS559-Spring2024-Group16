...
class weighted_LpLoss(object):
    ...
    def rel(self, x, y):
        diff = torch.norm(torch.flatten(x[...,:-5], start_dim=-self.d) - torch.flatten(y[...,:-5], start_dim=-self.d), \
                          p=self.p, dim=-1, keepdim=False)
        diff += 50 * torch.norm(torch.flatten(x[...,-5:], start_dim=-self.d) - torch.flatten(y[...,-5:], start_dim=-self.d), \
                          p=self.p, dim=-1, keepdim=False)
        ynorm = torch.norm(torch.flatten(y[...,:-5], start_dim=-self.d), p=self.p, dim=-1, keepdim=False)
        ynorm += 50 * torch.norm(torch.flatten(y[...,-5:], start_dim=-self.d), p=self.p, dim=-1, keepdim=False)
