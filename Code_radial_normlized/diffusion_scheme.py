...
def SchemeMatrix(N, h):
    ...
    A[0][0] = -2
    A[0][1] = 2
    A[N-1][N-2] = ((N + 0.5)**2 / (N**2)) + ((N - 0.5)**2 / (N**2))
    A[N-1][N-1] = - ((N + 0.5)**2 + (N - 0.5)**2) /(N**2)
        
    return A


def SolveDiffusionEquation(q_old, Boundary_condition_old, Boundary_condition_new, dt, h, L=1.0, theta=1.0):
    """
    --Args--
    ...
    M1 = np.linalg.inv( np.eye(N) - dt*A /(h**2) )
    q_old = np.dot(np.eye(N) + (1-theta)*dt*A, q_old) 
    q_old[-1] += (N + 0.5)**2 * dt*(Boundary_condition_new)  / (h**2) / N**2
    # M2 = np.dot(np.eye(N) , q_old) 

    q_new = np.dot(M1, q_old) 
